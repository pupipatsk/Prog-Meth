...
public class GameLogic {
    ...
    public boolean isHandPlayable(int playerIndex) {
    	...
    	ArrayList<BaseCard> hand = GameLogic.getInstance().getPlayerHand(playerIndex);
    	for (int i=0; i<hand.size(); i++) {
    		BaseCard topCard = GameLogic.getInstance().getTopCard();
    		if (hand.get(i).getColor()==topCard.getColor() || hand.get(i).getSymbol()==topCard.getSymbol() || hand.get(i).getColor()==null) {
    			return true;
    		}
    	}
    	return false;
    ...
    public ArrayList<BaseCard> draw(int amount) {
    	...
    	ArrayList<BaseCard> deck = GameLogic.getInstance().getDeck();
    	ArrayList<BaseCard> dummyArrayBaseCard = new ArrayList<BaseCard>();
    	if (deck.size() <= amount) {
    		return deck;
    	}
    	for (int i=0;i<amount;i++) {
    		dummyArrayBaseCard.add(deck.get(0));
    		deck.remove(0);
    	}
    	return dummyArrayBaseCard;
    }
    ...
    public int getPlayerCount() {
    	//getPlayerCount will return (int) number of people who still play (doesn't finish yet)
        int playerCount = 0;
