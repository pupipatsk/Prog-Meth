...
public class Main {
	...
	private static void skillFlow(ArrayList<BaseCompetitor> allCompetitors) {
		...
		for(int i = 0;i < allCompetitors.size();i++) {
			if(allCompetitors.get(i).getType()!="BaseCompetitor") {
		...
		while(true) {
			...
			else if(allCompetitors.get(skillUser).getType()!="BaseCompetitor")break;
			System.out.println("Invalid input");
			skillUser = sc.nextInt();
		}
		if(allCompetitors.get(skillUser).getType()=="Sorcerer") {
		...
		else if(allCompetitors.get(skillUser).getType()=="Tiger") {
		...
		else if(allCompetitors.get(skillUser).getType()=="ToughMan") {
